...
var addFeed, currentFeedUrl, errorHandler, feeds, fs, generateFeed, generateFolder, generateOverview, getFavicon, getJsonFeed, importFromOpml, init, refreshFeed, removeFeed, showContent, showDetail, showMenu, showSettingsPage, toggle, toggleAddBox, toggleMenu, toggleStar;
...
init = function() {
  ...
  for (_i = 0, _len = feeds.length; _i < _len; _i++) {
    item = feeds[_i];
    if (item.type === "rss") {
      _results.push(feed_ul.append(generateFeed(item)));
    } else {
...
importFromOpml = function(evt) {
  ...
  reader.onload = function(oFREvent) {
    var f, li, opml, outline, outline_str, sub_f, sub_outline, sub_outline_str, _i, _j, _len, _len1, _ref, _ref1, _results;
...
getFavicon = function(url) {
  ...
  return fs.root.getFile(domainName, {}, function(fileEntry) {
...
$(function() {
  ...
  return window.requestFileSystem(window.PERSISTENT, 1024 * 1024, function(filesystem) {
    return fs = filesystem;
  }, function(e) {
    return console.log("Could not request File System");
  });
});
